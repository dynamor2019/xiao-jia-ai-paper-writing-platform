...
选题先分别检索本专业问题和借鉴学科方法，再形成可检验的交叉迁移假设；报告列出文献 ID、迁移前提、适用边界、失效条件、数据路径和基线对照。机器复核只基于检索元数据与摘要，不能证明研究空白或替代阅读全文。Web 和 CLI 默认在选题后暂停；请先阅读论文目录下 `milestones/topic-discovery.md`，核对关键全文与真实数据可得性，再在 Web 使用 `/paper-confirm`，或在 CLI 使用原项目 ID 加 `--approval-mode auto` 继续。选题检索、模型或复核失败时不会生成固定备用题目，也不会进入写作。
