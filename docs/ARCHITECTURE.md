...
1. `F:\dsh` 是唯一可维护源码；`F:\DSH data` 是唯一论文输出、原始数据和运行状态根目录；用户目录中的 DSH 文件只是运行时部署副本。
...
论文工作台或 /paper
  ...
  -> `F:\DSH data\output\papers` 下当前对话专属目录中的 milestones/ 与 final/
```
...
- `F:\DSH data\.dsh-state/research-workspace.json`：项目主题、目标期刊和工作区元数据。
- `F:\DSH data\.dsh-state/web-paper-run.json`：Web 启动的后台进程状态，仅表示进程与当前阶段。
